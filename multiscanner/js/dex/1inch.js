                ...
                try {
                    ...
                    if (action === "TokentoPair") {
                        ...
                        if (quoteData) {
                            ...
                            feeSwap = parseFloat(quoteData.gasCostUSD) || getFeeSwap(chainName);
                        }
                    }

                    if (amount_out !== undefined) {
                        const result = { dexTitle: "1INCH", amount_out, FeeSwap: feeSwap };
                        callback(null, result);
                    } else {
                        throw new Error("Could not parse amount_out from 1inch response.");
                    }
                } catch (error) {
                    callback({ statusCode: 500, pesanDEX: `Error processing 1inch response: ${error.message}`, color: "#f39999", DEX: "1INCH" }, null);
                }
            },
            error: function(xhr) {
                callback({ statusCode: xhr.status, pesanDEX: `1inch API error: ${xhr.statusText}`, color: "#f39999", DEX: "1INCH" }, null);
            }
        });
    }
};
