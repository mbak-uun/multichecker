...
//   "pairContractAddress": "0x1f573d6fb3f13d689ff844b4ce37794d79a7ff1c",
//   "decimals": 18,
//   "pairDecimals": 18,
//   "chain": "Ethereum",
//   "modalCexToDex": {
...
//   },
//   "isActive": true,
//   "selectedCexs": ["GATE", "BINANCE"],
//   "selectedDexs": ["KyberSwap", "Matcha", "OKXDEX", "ODOS"]
// }
...
const DEX_URLS = {
    ...
    // magpie: ({ chainName, tokenSymbol, pairSymbol }) =>
    //     `https://app.magpiefi.xyz/swap/${chainName}/${tokenSymbol.toUpperCase()}/${chainName}/${pairSymbol.toUpperCase()}`,
    ...
    // paraswap: ({ chainName, tokenAddress, pairAddress }) =>
    //     `https://app.ParaSwap.xyz/#/swap/${tokenAddress}-${pairAddress}?version=6.2&network=${chainName}`,
...
var timeoutApi = LocalStorageUtil.get('SETTING_APP', {}).TimeoutCount || 5000;
...
const DEXAPIs = {
        ...
        return new Promise((resolve, reject) => {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    // Hitung feeSwap USDT (via localStorage GAS_INFO)
                    const feeSwapUSDT = PriceUtils.getGasFeeUSD(chainName || '', gasEstimate);
    ...
    get1inchQuotePrice: function(tokenIn, tokenOut, amountIn, chainId, chainName, userAddr) {
        return new Promise((resolve, reject) => {
            const gasInfoMap = LocalStorageUtil.get('GAS_INFO');
...
async function checkAllCEXWalletsPerChain() {
    ...
    for (const chainName in CEXWallets) {
        ...
        for (const cexName in walletList) {
            ...
            else if (cexKey === "GATE") allData = gateData;
   ...
   // console.log("🔧 Menyisipkan info wallet ke setiap token di SELECT_TOKENS...");

    const tokenMulti = LocalStorageUtil.get("SELECT_TOKENS");
    ...
    try {
        localStorage.setItem("SELECT_TOKENS", JSON.stringify(tokenMultiUpdated));
        console.log("✅ SELECT_TOKENS updated:", tokenMultiUpdated);
        ...
        $('#loadingOverlay').fadeOut(200);
        console.error("❌ Gagal menyimpan SELECT_TOKENS:", e);
        alert("❌ Gagal menyimpan SELECT_TOKENS: " + e.message);
    }
}
