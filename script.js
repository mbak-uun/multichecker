...
const STATE = {
    SET_APP:        LocalStorageUtil.get('SETTING_APP', {}),
    MASTER_KOIN:    LocalStorageUtil.get('MASTER_TOKENS', {}),
    CONFIG_SCAN:    LocalStorageUtil.get('CONFIG_SCANNER', {}),
    KOIN_SCANNER:   LocalStorageUtil.get('SELECT_KOIN', {}),
    GAS_INFO:       LocalStorageUtil.get('GAS_INFO', {}),
    LOG_ACTION:     LocalStorageUtil.get('HISTORTY_ACTION', {}),
    SELECT_THEME:   LocalStorageUtil.rawGet('SELECT_THEME', {}),
    AUTO_SCROLL:    LocalStorageUtil.get('AUTO_SCROLL', {})
};
...
//        this.tokens = this.loadTokens();
    ...
    checkAppRequirements() {
        const requirements = [
        { key: 'SETTING_APP', data: this.state.SET_APP, message: '⚠️ SILAKAN SETTING APLIKASI TERLEBIH DAHULU.' },
        { key: 'MASTER_TOKENS', data: this.state.MASTER_KOIN, message: '⚠️ SILAKAN SINKRONISASI DATA KOIN !!!' },
        { key: 'CONFIG_SCANNER', data: this.state.CONFIG_SCAN, message: '⚠️ SILAKAN KONFIGURASI KOIN SCANNER' },
        { key: 'SELECT_KOIN', data: this.state.KOIN_SCANNER, message: '⚠️ SILAKAN TENTUKAN KOIN ( MEMILIH CHAIN & EXCHANGER )' }
        ];
    ...
    init() {
        ...
        this.bindEvents();
       // ✅ Cek kelengkapan data sebelum lanjut
        ...
        //     if (dataEmpty) {
        //         this.showAlert(item.message, 'danger');
        //         $('#mainTabs a[href="#appSettings"]').tab('show');
        ...
        const lastWalletUpdate = localStorage.get("HISTORTY_ACTION");
    ...
    initSettingScan() {
        ...
        const storedConfig = LocalStorageUtil.get('CONFIG_SCANNER') || {};

        // Reset semua checkbox CHAIN & CEX
        $('#chainCheckboxGroup input[name="set_chain"]').prop('checked', false);
        $('#cexCheckboxGroup input[name="set_cex"]').prop('checked', false);

        // ✅ Centang CHAIN sesuai simpanan
        (storedConfig.chains || []).forEach(val => {
            $(`#chainCheckboxGroup input[name="set_chain"][value="${val}"]`).prop('checked', true);
        });

        // ✅ Centang CEX sesuai simpanan
        const selectedCexNames = (storedConfig.cexs || []).map(c => c.name);
        selectedCexNames.forEach(name => {
            $(`#cexCheckboxGroup input[name="set_cex"][value="${name}"]`).prop('checked', true);
        });

        // ✅ Jalankan updateSelectedCoins agar checkbox CEX–CHAIN muncul berdasarkan CHAIN dan CEX tercentang
        this.updateSelectedCoins();

        // ✅ Centang CEX–CHAIN sesuai data simpanan
        setTimeout(() => {
            // Reset semua checkbox CEX–CHAIN dulu
            $('input[name="set_cex_chain"]').prop('checked', false);

            (storedConfig.cexs || []).forEach(cex => {
                (cex.chains || []).forEach(chain => {
                    const val = `${cex.name}|${chain}`;
                    $(`input[name="set_cex_chain"][value="${val}"]`).prop('checked', true);
                });
            });
        }, 100); // Tunggu DOM hasil updateSelectedCoins

        // ✅ Bind ulang event agar updateSelectedCoins terpicu saat user klik checkbox
        $('#chainCheckboxGroup input[name="set_chain"]').off('change').on('change', this.updateSelectedCoins);
        $('#cexCheckboxGroup input[name="set_cex"]').off('change').on('change', this.updateSelectedCoins);
    }
    ...
    bindEvents() {
        ...
        $('#saveScanSettingsBtn').on('click', (e) => {
            ...
            LocalStorageUtil.set('CONFIG_SCANNER', settingConfig);

            // 5. Ambil semua token master
            const masterTokens = LocalStorageUtil.get('MASTER_TOKENS');
            ...
            LocalStorageUtil.set('SELECT_KOIN', selectedTokens);

            // Logging dan konfirmasi
            if (typeof this?.logAction === 'function') this.logAction(`SETTING SCANNER`);
            alert(`✅ SIMPAN KONFIGURASI SCANNER BERHASIL!\n 🪙 TERPILIH ${selectedTokens.length} KOIN`);
            location.reload();
        });

        $('#ModalConfigScan').on('show.bs.modal', () => {
            const config = LocalStorageUtil.get('CONFIG_SCANNER');
            if (!config) return;

            $('#ConfigScanForm input[type="checkbox"]').prop('checked', false);

            (config.chains || []).forEach(val => {
                $(`#ConfigScanForm input[name="chain"][value="${val}"]`).prop('checked', true);
            });

            (config.cexs || []).forEach(val => {
                $(`#ConfigScanForm input[name="cex"][value="${val}"]`).prop('checked', true);
            });

            // ✅ Sekarang this benar-benar class instance
            this.updateCexCountsBySelectedChains();

             // ✅ Tambah listener ke CHAIN dan CEX
            $('#ConfigScanForm input[name="chain"]').off('change').on('change', this.updateCexCountsBySelectedChains);
            $('#ConfigScanForm input[name="cex"]').off('change').on('change', this.updateCexCountsBySelectedChains);

        });

        $('#saveSyncKoinBtn').on('click', async (event) => {
            const selectedChains = $('#SyncKoinForm input[name="sync_chain"]:checked')
                .map(function () { return this.value.toLowerCase(); }).get();

            if (selectedChains.length === 0) {
                alert("⚠️ Pilih minimal satu chain.");
                return;
            }

            const selectedDexs = $('#SyncKoinForm input[name="sync_dex"]:checked')
                .map(function () { return this.value; }).get();

            if (selectedDexs.length === 0 || selectedDexs.length > 4) {
                alert("⚠️ Pilih minimal 1 sampai maksimal 4 DEX.");
                return;
            }

            const selectedCexs = $('#SyncKoinForm input[name="sync_cex"]:checked')
                .map(function () { return this.value.toLowerCase(); }).get();

            if (selectedCexs.length === 0) {
                alert("⚠️ Pilih minimal 1 CEX.");
                return;
            }

            const modalCexToDex = parseFloat($('#sync_modal_cex_dex').val()) || 0;
            const modalDexToCex = parseFloat($('#sync_modal_dex_cex').val()) || 0;

            if (modalCexToDex <= 0 || modalDexToCex <= 0) {
                alert('⚠️ Modal harus diisi (lebih dari 0).');
                return;
            }

            // ❄️ Tampilkan loading freeze
            ...
            for (const chain of selectedChains) {
                ...
                try {
                    ...
                    tokens.forEach(t => {
                        ...
                        if (!match) return; // ⛔️ skip token jika cex-nya tidak terpilih

                       // Konversi jsonCex (dari lowercase atau nama tidak rapi) jadi uppercase
                        const upperCex = jsonCex.toUpperCase();

                        // Cek apakah ada di CONFIG_CEX
            ...
            this.logAction(`SIKRONISASI DATA`);
            // Tampilkan alert sukses
            alert(`✅ Berhasil SYNC DATA${allTokenData.length} KOIN. \n SILAKAN KLIK "OK" UNTUK RELOAD HALAMAN`);
            location.reload()
            // Tutup modal
       ...
       $('#CheckPrice').on('click', async () => {
            ...
            let config = LocalStorageUtil.get('CONFIG_SCANNER');

            // Jika config tidak valid, buat default
            ...
            // Jika tidak ada properti run, tambahkan
            if (!config.hasOwnProperty('run')) {
                config.run = "YES";
                LocalStorageUtil.set('CONFIG_SCANNER', config);
            }

            // Jika run sudah YES, beri peringatan
            if (config.run === "YES") {
                alert("⚠️ SCAN MASIH JALAN..SILAKAN STOP DAHULU");
                return;
    }

            // 🔃 Jika belum jalan, lanjutkan
            config.run = "YES";
            LocalStorageUtil.rawSet('CONFIG_SCANNER', JSON.stringify(config));
           ...
           // Nonaktifkan klik pada tab lain (bukan disembunyikan)
            ...
            this.sendStatusTELE(this.parsedSettings.UserName, 'ONLINE');
        ...
        $('#StopScan').on('click', () => {
            ...
            const confirmConfig = LocalStorageUtil.get('CONFIG_SCANNER');
            if (confirmConfig.run === "NO") {
                alert("✅ SUKSES STOP SCAN");
            } else {
                alert("❌ GAGAL STOP SCAN");
            }
            ...
            setTimeout(() => location.reload(), 300); // ✅ Jeda sebelum reload
        });
      ...
      //  Save token button
        ...
        //  Save setting app button
        $('#settingsForm').on('submit', (e) => {
            e.preventDefault(); // cegah reload form
            this.saveSettings(); // ganti sesuai fungsi simpan Anda
        });
        ...
        $('#importTokensInput').on('change', (e) => {
            ...
            reader.onload = (event) => {
                ...
                LocalStorageUtil.set('SELECT_KOIN',tokens);
                LocalStorageUtil.set('MASTER_TOKENS', tokens);
                app.tokens = tokens;
                app.loadTokenTable();
                app.updateStats();
                autoDetectChainsAndCEX(tokens);
        ...
        function autoDetectChainsAndCEX(tokens) {
            ...
            tokens.forEach(token => {
                ...
                // Ambil dari selectedCexs (karena di CSV tidak ada field token.cex)
            ...
            const configScan = {
                ...
                cexs: Array.from(cexs),
                run: "NO"
            };

            // Simpan ke localStorage
            LocalStorageUtil.rawSet('CONFIG_SCANNER', JSON.stringify(configScan));
            ...
            chains.forEach(chain => {
                $(`#SettingScan input[name="set_chain"][value="${chain}"]`).prop('checked', true);
            });

            cexs.forEach(cex => {
                $(`#SettingScan input[name="set_cex"][value="${cex}"]`).prop('checked', true);
            });

            console.log('✅ Auto-detect selesai. Chains & CEXs:', configScan);
        }
   ...
   initPairSymbolAutocomplete() {
    ...
    InfoConfigScan() {
    //    console.log("MASTER TOKEN", STATE.MASTER_KOIN);
    //    console.log('CONFIG SCAN:', STATE.CONFIG_SCANNER);
        const config = LocalStorageUtil.get('CONFIG_SCANNER');
        const selectedTokens = LocalStorageUtil.get('SELECT_KOIN');
    ...
    saveSettings() {
        ...
        if (!userName) {
            this.showAlert('❌ Nama pengguna tidak boleh kosong', 'danger');
            return;
        }
        ...
        if (!timeoutCount || timeoutCount < 2000 || timeoutCount > 10000) {
            this.showAlert('Timeout harus antara 2000–10000 ms', 'danger');
            return false;
        }
        ...
        alert('✅ SIMPAN SETINGAN APLIKASI BERHASIL!');
        location.reload();
    }

    async fetchGasTokenPrices() {
        const configSCAN = LocalStorageUtil.rawGet('CONFIG_SCANNER');
        ...
        let response = null;
        try {
            response = await $.getJSON(binanceURL);
        } catch (err) {
            console.error('❌ Error fetch harga token:', err);
        ...
        response.forEach(item => {
            ...
            if (symbol === 'BTC') $('#btcPrice').text(`$${price.toFixed(2)}`);
            if (symbol === 'BNB') $('#bnbPrice').text(`$${price.toFixed(2)}`);
            if (symbol === 'ETH') $('#ethPrice').text(`$${price.toFixed(2)}`);
            if (symbol === 'MATIC') $('#maticPrice').text(`$${price.toFixed(2)}`);
        });
        ...
        // 🔍 Ambil selected chains dari localStorage CONFIG_SCANNER
        // let selectedChains = [];
        // try {
        //     const config = JSON.parse(configSCAN);
        //     if (Array.isArray(config?.chains)) {
        //         selectedChains = config.chains;
        //     }
        // } catch (e) {
        //     console.warn("❗ CONFIG_SCANNER tidak valid:", e);
        // }
        ...
        try {
            ...
            // Parsing jika hasil dari rawGet adalah string JSON
            if (typeof config === 'string') {
                config = JSON.parse(config);

                // Jika setelah parse ternyata masih string (double encode), parse lagi
                if (typeof config === 'string') {
                    config = JSON.parse(config);
                }
            }
            ...
            console.warn("❗ Parsing CONFIG_SCANNER gagal:", e);
        }

        // Debugging opsional
        //console.log("📋 Selected Chains:", selectedChains);
        //console.log("📦 CHAIN_CONFIG Names:", Object.values(CHAIN_CONFIG).map(v => v.name));

        // 🧩 Mapping selected chain ke CHAIN_CONFIG (gunakan name dengan trim + lowercase)
        const matchedChains = Object.entries(CHAIN_CONFIG)
            .filter(([key, val]) => {
                const valName = (val.name || "").trim().toLowerCase();
                return selectedChains.some(sel => (sel || "").trim().toLowerCase() === valName);
            })
            .map(([key, val]) => ({
                key,
                label: val.short,
        ...
        if (matchedChains.length === 0) {
            $('#gasTokenPrices').html(`<span class="text-warning">Tidak ada chain terpilih.</span>`);
            throw new Error("Tidak ada chain yang dipilih di CONFIG_SCANNER");
        }

        let successCount = 0;

        for (const chain of matchedChains) {
            const { key, label, symbol, gasLimit, rpc } = chain;
            let tokenPrice = tokenPrices[symbol];

            // 🔄 Mapping manual untuk POL → MATIC
            if (!tokenPrice && symbol === 'POL') {
                tokenPrice = tokenPrices['MATIC'];
            }

            if (!rpc || !tokenPrice) {
                gasTextParts.push(`<span class='badge bg-light text-dark fs-8'>${label} [n/a]</span>`);
                continue;
            }
            ...
            try {
                ...
                const gwei = (baseFee / 1e9) * 2; // +100% buffer
                const gasUSDT = (gwei * gasLimit * tokenPrice) / 1e9;

                console.log(`🟢 ${label} | Gwei: ${gwei.toFixed(2)} | Token: ${symbol} | Price: $${tokenPrice} | Fee ≈ $${gasUSDT.toFixed(4)}`);

                gasTextParts.push(
                    `<span class='badge bg-secondary text-white fs-8 fw-bold'>🔥 ${label} [${gwei.toFixed(2)} | $${gasUSDT.toFixed(4)}]</span>`
                );
                ...
                console.error(`❌ Gagal ambil gas untuk ${label}:`, err);
                gasTextParts.push(`<span class='badge bg-danger fs-8'>${label} [err]</span>`);
            }
        }

        $('#gasTokenPrices').html(gasTextParts.join(" "));
        LocalStorageUtil.set('GAS_INFO', gasTokenInfo);
   ...
   // Token management
    ...
    async CheckPrices() {
        ...
        for (const token of this.tokens) {
            ...
            if (!selectedChains.includes(token.chain)) continue;

            const matchedCexs = token.selectedCexs?.filter(cex => selectedCexs.includes(cex)) || [];
            for (const cexName of matchedCexs) {
        ...
        if (allTokenUnits.length === 0) {
            this.showAlert('Tidak Ada Token Aktif untuk Config Saat Ini', 'info');
        ...
        for (const batch of unitBatches) {
            await Promise.allSettled(batch.map(async tokenUnit => {
                ...
                if (firstToken) this.tryAutoScrollToRow(firstToken);
                    ...
                    .map(async dexName => {
                        ...
                        if (this.errorStats) {
                            const errorSummary = Object.entries(this.errorStats)
                                .map(([dex, stat]) =>
                                    `<span class="badge text-dark bg-warning fs-8">❌ ${dex.toUpperCase()} [ 🕒: ${stat.timeout || 0} | ⚠️: ${stat.dexError || 0} ]</span><br/>`
                                ).join('');
        ...
        // 🔚 Akhiri: tandai config.run = false + enable tombol
        config.run = "NO";
        LocalStorageUtil.rawSet('CONFIG_SCANNER', JSON.stringify(config));
        $('#CheckPrice').prop('disabled', false);
    }
                ...
                case 'Gate':
                    promises.push(
                        CEXAPIs.getGateOrderBook(pair).then(assignData).catch(err =>
                            console.warn(`Gate ${symbol}/USDT error: ${err.message}`)
                        )
                    );
                    break;
                case 'INDODAX':
           ...
           // console.warn(dexName,data);
            ...
            if (direction === 'cex_to_dex') {
                ...
                */
                ...
                // }
                if (pnl > parseFloat(this.parsedSettings.PNLFilter)) {
                    // const alertMsg = `
                    //     🌐 CHAIN: ${token.chain.toUpperCase()} | 
...
//                'ParaSwap': 'paraswap',
           ...
           // this.highestPNLSignal = this.highestPNLSignal || {};
            ...
            if (!this.pnlSignals[dexKey][signalKey] && !existingLI) {
                ...
                if (listEl) {
                    ...
                    const highlightClass = parseFloat(pnlNetto) > parseFloat(this.parsedSettings.PNLFilter)
            ...
            const username = this.parsedSettings.UserName || 'Anon';
            const modalParsed = parseFloat(modal);
            const pnlFilter = parseFloat(this.parsedSettings.PNLFilter);
      ...
      return `
          <div class="bg-white py-1">
                ...
                <div class="d-block mb-1">   
                    <button class="btn btn-xs badge bg-success" onclick="app.confirmToggleToken('${token.id}')" title="Ganti Status">
                        ⏻
                    </button>
                    <strong class="fs-7 align-middle">${token.modalCexToDex}$ ⇔ ${token.modalDexToCex}$</strong>
                    <button class="btn btn-xs badge bg-light" onclick="app.editToken('${token.id}')" title="Ubah">
                        ✏️
                    </button>
                    <button class="btn btn-xs badge bg-danger" onclick="app.deleteToken('${token.id}')" title="Hapus">
                        🗑️
                    </button>
                </div>
    ...
    tryAutoScrollToRow(firstToken) {
        const isAutoScroll = localStorage.getItem('AUTO_SCROLL') === 'true';
    ...
    renderFormSettingScan() {
        const $chainContainer = $('#chainCheckboxGroup').empty();
        const $cexContainer = $('#cexCheckboxGroup').empty();
        ...
        // === CEK MASTER TOKEN ===
        const tokenList = LocalStorageUtil.get('MASTER_TOKENS', []);
            ...
            $('#scanSettingInfo').html(`
                <div class="alert alert-warning d-flex justify-content-between align-items-center py-2 px-3 mb-0">
                    <div>
                        ...
                        <strong>TIDAK ADA DATA TOKEN!</strong> Silakan Sync Token terlebih dahulu.
                    </div>
                </div>
            `);
                    // Disable semua checkbox jika data token kosong
            $('#chainCheckboxGroup input[type="checkbox"]').prop('disabled', true);
            $('#cexCheckboxGroup input[type="checkbox"]').prop('disabled', true);

        } else {
            $('#scanSettingInfo').html(''); // clear jika token tersedia
        }
    }
    ...
    updateSelectedCoins() {
        const selectedChains = $('#chainCheckboxGroup input[name="set_chain"]:checked')
            .map(function () { return this.value; }).get();

        const tokens = LocalStorageUtil.get('MASTER_TOKENS', []);

        const activeTokens = tokens.filter(t => t.isActive && selectedChains.includes(t.chain));

        // console.log("⛓️ selectedChains:", selectedChains);
        // console.log("📦 MASTER_TOKENS from localStorage:", tokens);
        // console.log("🔍 activeTokens after filter:", activeTokens);

        const cexChainCounts = {};
        const chainCounts = {};

        activeTokens.forEach(t => {
            (t.selectedCexs || []).forEach(cex => {
                if (!cexChainCounts[cex]) cexChainCounts[cex] = {};
                if (!cexChainCounts[cex][t.chain]) cexChainCounts[cex][t.chain] = 0;
                cexChainCounts[cex][t.chain]++;
            });

            chainCounts[t.chain] = (chainCounts[t.chain] || 0) + 1;
        });

        $('#chainCheckboxGroup input[name="set_chain"]').each(function () {
            const val = $(this).val();
            const label = $(`label[for="${$(this).attr('id')}"]`);
            const count = chainCounts[val] || 0;
            label.html(`${val.toUpperCase()} <span class="badge bg-primary">${count}</span>`);
        });

        $('#cexCheckboxGroup input[name="set_cex"]').each(function () {
            const cex = $(this).val();
            const label = $(`label[for="${$(this).attr('id')}"]`);
        ...
        // Pastikan semua checkbox hasil generate juga ikut dicentang
        $('input[name="set_cex_chain"]').prop('checked', true);

        const selectedCexChains = $('input[name="set_cex_chain"]:checked')
            .map(function () {
                const [cex, chain] = this.value.split('|');
                return { cex, chain };
            }).get();

        const selectedCexChainMap = {};
        selectedCexChains.forEach(({ cex, chain }) => {
            if (!selectedCexChainMap[cex]) selectedCexChainMap[cex] = [];
            selectedCexChainMap[cex].push(chain);
        });

        // Anda bisa gunakan selectedCexChainMap untuk logika lanjut
    }
...
function applyTheme(theme) {
    const validTheme = isValidTheme(theme) ? theme : 'biru';

    $('body').attr('data-theme', validTheme);
    LocalStorageUtil.rawSet('SELECT_THEME', validTheme);

    $('.theme-box').removeClass('active');
    $(`.theme-box[data-theme="${validTheme}"]`).addClass('active');
}

$(document).ready(function() {

    // if (STATE.CONFIG_SCANNER.run === "YES") {
    //     STATE.CONFIG_SCANNER.run = "NO";
    //     LocalStorageUtil.rawSet('CONFIG_SCANNER', STATE.CONFIG_SCANNER);
    //     //console.warn("🔁 Halaman direfresh saat scan aktif.");
    // }
    ...
    const savedTheme = LocalStorageUtil.rawGet('SELECT_THEME') || 'biru';
    ...
    $('#autoScrollCheckbox').prop('checked', LocalStorageUtil.rawGet('AUTO_SCROLL') === 'true');

    $('#autoScrollCheckbox').on('change', function () {
        const isChecked = $(this).is(':checked');
        LocalStorageUtil.set('AUTO_SCROLL', isChecked);
